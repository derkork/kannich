# Changelog
All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.6.0] - 2026-08-28
### Added
- NPM global cache is now properly held in the Kannich cache.

## [0.5.0] - 2026-05-15
### Added
- ARM64 is now fully supported. The correct Node.js binary for the current architecture is automatically downloaded.

### Deprecated
- `home()` is deprecated. Use `getInstallPath()` instead.

## [0.4.0] - 2026-03-20
### Changed
- Bumped the `kannich-parent` dependency to `0.9.0`.
- Removed unnecessary dependencies slowing down module loading.

## [0.3.0] - 2026-03-06
### Added
- `Node` now supports the standardized `Tool` execution interface.

## [0.2.0] - 2026-03-05
### Added
- The `exec` command now accepts a `silent` flag which prevents the output from appearing in the Kannich logs. It also returns an `ExecResult` object containing the command's output and exit code.

## [0.1.0] - 2026-03-04
- Initial release